<template>
	<div>
		...
		<div class="card border-left-warning shadow h-100 py-2 mb-4" v-show="pendingRequests">
			<div class="card-body">
				<div class="row no-gutters align-items-center">
					...
					<div class="col-auto">
						<router-link class="d-none d-sm-inline-block btn btn-sm btn-warning shadow-sm" :to="{ name: 'AdminMembersRequests' }"><i class="fas fa-eye mr-2"></i> Check them</router-link>
					</div>
				</div>
			</div>
		</div>
		...
		<!-- Affiliate -->
		<div class="card shadow h-100 py-2 mb-4" :class="{ 'border-left-danger' : organization.affiliate == 0, 'border-left-primary' : organization.affiliate == 1}" ref="affiliateContainer">
			<div class="card-body">
				<div class="row no-gutters align-items-center">
					<div class="col mr-2">
						<div class="text-xs font-weight-bold text-uppercase mb-1" :class="{ 'text-danger' : organization.affiliate == 0, 'text-primary' : organization.affiliate == 1}">Affiliate</div>
						<div class="h5 mb-1 font-weight-bold text-gray-800">Your organization cannot be accessible by anyone on our portal.</div>
						<div class="h6 mb-0 text-gray-600">Affiliation gives your organization credibility in the world from ILA. Also, makes users find your content easily + other benefits.</div>
					</div>
					<div class="col-auto">
						<button type="button" v-if="organization.affiliate == 0" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm" @click="toggleAffiliation"><i class="fas fa-thumbs-up mr-2"></i> Enable Affiliation</button>
						<button type="button" v-else class="d-none d-sm-inline-block btn btn-sm btn-danger shadow-sm" @click="toggleAffiliation"><i class="fas fa-thumbs-down mr-2"></i> Disable Affiliation</button>
					</div>
				</div>
			</div>
		</div>
							...
							<div class="card-body">
								<div class="row no-gutters align-items-center">
									<div class="col mr-2">
										...
										<div class="h5 mb-1 font-weight-bold text-gray-800">Choose members in your organization to display on your ILA site.</div>
...
<script>
	...
	export default {
		...
		methods: {
	        ...
	        toggleAffiliation() {
	        	const loader = this.$loading.show({
	        		container: this.$refs.affiliateContainer
	        	})

	        	axios.put('/api/organization/affiliate/')
	        	.then(response => {
	        		Toast.fire({
	        			type: 'success',
	        			title: 'Affiliate settings changed!'
	        		})
	        	...
	        	.catch(err => {
	        		Toast.fire({
	        			type: 'error',
	        			title: 'We cannot change your affiliate settings at the moment'
	        		})
	        	})
	        	.finally(() => {
	        		loader.hide()
		...
		created() {
			document.title = 'Admin Dashboard | ILA'
